  ...
  try {
    ...
    if (action === "load") {
      const { data: links, error: linksError } = await supabaseAdmin
        .from("page_links")
        .select("public_id,link_type,title,subtitle,url,sort_order,highlighted,button_color")
      ...
      return json(request, {
        page: {
          ...
          presentationText: page.presentation_text, primaryColor: page.primary_color, logoUrl: safeMediaUrl(page.logo_url),
          coverUrl: safeMediaUrl(page.cover_url), coverType: page.cover_type,
    ...
    if (action === "contact") {
      ...
      const configuredFields = Array.isArray(config.fields) ? config.fields.filter((field): field is Record<string, unknown> => Boolean(field) && typeof field === "object") : [];
      const valueFor = (key: string) => key === "full_name" ? fullName : key === "whatsapp" ? whatsapp : key === "email" ? email : clean(fieldValues[key], 300) || null;
      const missingRequired = configuredFields.find((field) => field.required === true && !valueFor(clean(field.key, 40)));
      if (missingRequired) return json(request, { error: `Informe ${clean(missingRequired.label, 80) || "todos os campos obrigatórios"}.` }, 400);
      if (!whatsapp && !email && !clean(fieldValues.phone, 40)) return json(request, { error: "O formulário precisa receber WhatsApp, telefone ou e-mail." }, 400);
      ...
      const birthDate = /^\d{4}-\d{2}-\d{2}$/.test(clean(fieldValues.birth_date, 10)) ? clean(fieldValues.birth_date, 10) : null;
      const { data: captured, error: contactError } = await supabaseAdmin.from("captured_contacts").insert({
        organization_id: page.organization_id, account_id: page.account_id, smart_page_id: page.id,
        source, full_name: fullName, whatsapp, email, birth_date: birthDate, field_values: fieldValues,
