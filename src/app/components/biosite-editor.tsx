...
function PhonePreview({ page, draft, links, publicPath }: { page: SmartPage; draft: PageDraft; links: PageLink[]; publicPath: string }) {
  ...
  return <aside className="h-fit xl:sticky xl:top-24"><div className="mb-3 flex items-center justify-between"><div><p className="text-xs font-bold uppercase tracking-wider text-slate-400">Pré-visualização</p><p className="mt-1 text-[10px] text-slate-400">/{page.slug}</p></div><div className="flex gap-1"><button onClick={() => void copy()} className="rounded-lg border border-slate-200 p-2 text-slate-500" title="Copiar link"><Copy className="size-4" /></button>{page.status === "published" && <a href={publicPath} target="_blank" rel="noreferrer" className="rounded-lg border border-slate-200 p-2 text-slate-500" title="Abrir página"><ExternalLink className="size-4" /></a>}</div></div><div className="mx-auto max-w-[350px] overflow-hidden rounded-[36px] border-[8px] border-slate-950 shadow-2xl" style={backgroundStyle(draft.background_mode, draft.background_value)}><div className="relative h-40 overflow-hidden bg-slate-300">{draft.cover_url && draft.cover_type === "video" ? <video src={draft.cover_url} className="size-full object-cover" muted autoPlay loop playsInline /> : draft.cover_url ? <div className="size-full bg-cover bg-center" style={{ backgroundImage: `url(${draft.cover_url})` }} /> : <div className="grid size-full place-items-center text-slate-500"><ImageIcon className="size-8" /></div>}</div><div className="px-5 pb-6"><div className="-mt-10 size-20 rounded-full border-4 border-white bg-slate-900 bg-contain bg-center bg-no-repeat shadow-md" style={draft.logo_url ? { backgroundImage: `url(${draft.logo_url})` } : undefined}>{!draft.logo_url && <span className="grid size-full place-items-center text-lg font-bold text-white">{draft.name.slice(0, 2).toUpperCase()}</span>}</div><h3 className="mt-3 text-xl font-bold text-slate-950">{draft.name}</h3><p className="mt-1 text-xs text-slate-600">{draft.short_description || "Subtítulo da página"}</p><div className="mt-5 space-y-2">{links.filter((link) => link.active && !socialTypes.includes(link.link_type)).map((link) => { const color = link.button_color || (link.highlighted ? draft.highlight_color : draft.button_color); const filled = link.highlighted || draft.button_variant === "filled"; return <div key={link.id} className={`flex min-h-11 items-center justify-center gap-2 px-4 text-center text-xs font-bold ${radius}`} style={filled ? { backgroundColor: color, color: "white" } : { borderColor: color, borderWidth: draft.button_border_width, color }}><IconFor type={link.link_type} />{link.title}</div>; })}</div><div className="mt-4 flex justify-center gap-3 text-slate-700">{links.filter((link) => link.active && socialTypes.includes(link.link_type)).map((link) => <IconFor key={link.id} type={link.link_type} className="size-5" />)}</div>{draft.capture_enabled && <div className="mt-5 rounded-2xl bg-white/85 p-4 shadow-sm backdrop-blur"><strong className="text-sm">{draft.capture_config.title || "Quero receber novidades"}</strong><div className="mt-3 space-y-2">{formFields.map((field) => <div key={field.key} className="h-9 rounded-lg border border-slate-200 bg-white px-3 py-2 text-[10px] text-slate-400">{field.label}{field.required ? " *" : ""}</div>)}</div><div className="mt-3 rounded-lg py-2.5 text-center text-[10px] font-bold text-white" style={{ backgroundColor: draft.form_button_color }}>{draft.capture_config.button_text || "Quero me cadastrar"}</div></div>}<p className="mt-5 text-center text-[9px] text-slate-500">{draft.footer_text || "Powered by OctaReview"}</p></div></div></aside>;
}
