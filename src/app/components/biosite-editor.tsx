...
import {
  ArrowDown, ArrowUp, BriefcaseBusiness, CalendarDays, Camera, Check, Copy, ExternalLink, FileDown,
  Gift, Globe2, GripVertical, ImageIcon, Link2, MapPin, Play, Users,
  MessageCircle, Music2, Palette, Plus, Save, ShoppingBag, Star, Trash2, Upload,
...
export type PageDraft = Pick<SmartPage,
  ...
  "button_border_width" | "footer_text" | "capture_enabled" | "capture_config"
>;
...
type Props = {
  ...
  onMoveLink: (link: PageLink, direction: -1 | 1) => void;
  ...
  return <div className="space-y-5">
    ...
    <div className="grid gap-5 xl:grid-cols-[190px_minmax(0,1fr)_360px]">
      ...
      <div className="min-w-0 space-y-5">
        ...
        {section === "buttons" && <><SectionCard title="Estilo dos botões" detail="Os ícones são definidos automaticamente conforme o tipo do botão."><div className="grid gap-4 sm:grid-cols-2"><ColorControl label="Cor dos botões" value={draft.button_color} onChange={(value) => patch("button_color", value)} /><ColorControl label="Cor de destaque" value={draft.highlight_color} onChange={(value) => patch("highlight_color", value)} /></div><div className="mt-5 grid gap-4 sm:grid-cols-3"><Choice label="Formato" value={draft.button_shape} options={[["square", "Reto"], ["soft", "Leve"], ["round", "Redondo"]]} onChange={(value) => patch("button_shape", value as PageDraft["button_shape"])} /><Choice label="Aparência" value={draft.button_variant} options={[["filled", "Preenchido"], ["outline", "Contorno"]]} onChange={(value) => patch("button_variant", value as PageDraft["button_variant"])} /><Choice label="Espessura" value={String(draft.button_border_width)} options={[["1", "1 px"], ["2", "2 px"], ["3", "3 px"]]} onChange={(value) => patch("button_border_width", Number(value) as 1 | 2 | 3)} /></div></SectionCard><SectionCard title="Botões" detail="Adicione, ordene, oculte ou destaque os atalhos da página."><form onSubmit={props.onAddLink} className="grid gap-2 sm:grid-cols-[150px_1fr_1.4fr_auto]"><select name="type" className={inputClass}>{buttonTypes.map((type) => <option key={type} value={type}>{type}</option>)}</select><input required name="title" placeholder="Texto do botão" className={inputClass} /><input required name="url" type="url" placeholder="https://" className={inputClass} /><button disabled={busy} className="grid size-11 place-items-center rounded-xl bg-slate-950 text-white"><Plus className="size-4" /></button></form><div className="mt-4 space-y-2">{nonSocialLinks.map((link, index) => <div key={link.id} className={`flex items-center gap-2 rounded-xl border p-2 ${link.highlighted ? "border-teal-200 bg-teal-50" : "border-slate-100"}`}><GripVertical className="size-4 text-slate-300" /><span className="grid size-8 place-items-center rounded-lg bg-slate-100 text-slate-600"><IconFor type={link.link_type} /></span><strong className="min-w-0 flex-1 truncate text-xs">{link.title}</strong><button title="Visibilidade" onClick={() => props.onUpdateLink(link, { active: !link.active })} className={`h-5 w-9 rounded-full p-0.5 ${link.active ? "bg-teal-600" : "bg-slate-300"}`}><span className={`block size-4 rounded-full bg-white transition ${link.active ? "translate-x-4" : ""}`} /></button><button title="Destaque" onClick={() => props.onUpdateLink(link, { highlighted: !link.highlighted })} className={link.highlighted ? "text-teal-600" : "text-slate-300"}><Star className="size-4" fill={link.highlighted ? "currentColor" : "none"} /></button><button disabled={index === 0} onClick={() => props.onMoveLink(link, -1)} className="text-slate-400 disabled:opacity-20"><ArrowUp className="size-4" /></button><button disabled={index === nonSocialLinks.length - 1} onClick={() => props.onMoveLink(link, 1)} className="text-slate-400 disabled:opacity-20"><ArrowDown className="size-4" /></button><button onClick={() => props.onDeleteLink(link)} className="text-slate-400 hover:text-rose-600"><Trash2 className="size-4" /></button></div>)}</div></SectionCard></>}
...
function PhonePreview({ page, draft, links, publicPath }: { page: SmartPage; draft: PageDraft; links: PageLink[]; publicPath: string }) {
  ...
  return <aside className="h-fit xl:sticky xl:top-24"><div className="mb-3 flex items-center justify-between"><div><p className="text-xs font-bold uppercase tracking-wider text-slate-400">Pré-visualização</p><p className="mt-1 text-[10px] text-slate-400">/{page.slug}</p></div><div className="flex gap-1"><button onClick={() => void copy()} className="rounded-lg border border-slate-200 p-2 text-slate-500" title="Copiar link"><Copy className="size-4" /></button>{page.status === "published" && <a href={publicPath} target="_blank" rel="noreferrer" className="rounded-lg border border-slate-200 p-2 text-slate-500" title="Abrir página"><ExternalLink className="size-4" /></a>}</div></div><div className="mx-auto max-w-[350px] overflow-hidden rounded-[36px] border-[8px] border-slate-950 shadow-2xl" style={backgroundStyle(draft.background_mode, draft.background_value)}><div className="relative h-40 overflow-hidden" style={!draft.cover_url ? backgroundStyle(draft.background_mode, draft.background_value) : undefined}>{draft.cover_url && draft.cover_type === "video" ? <video src={draft.cover_url} className="size-full object-cover" muted autoPlay loop playsInline /> : draft.cover_url ? <div className="size-full bg-cover bg-center" style={{ backgroundImage: `url(${draft.cover_url})` }} /> : null}</div><div className="px-5 pb-6"><div className="flex flex-col items-center text-center"><div className="-mt-10 size-20 rounded-full border-4 border-white bg-slate-900 bg-contain bg-center bg-no-repeat shadow-md" style={draft.logo_url ? { backgroundImage: `url(${draft.logo_url})` } : undefined}>{!draft.logo_url && <span className="grid size-full place-items-center text-lg font-bold text-white">{draft.name.slice(0, 2).toUpperCase()}</span>}</div><h3 className="mt-3 text-xl font-bold text-slate-950">{draft.name}</h3><p className="mt-1 text-xs text-slate-600">{draft.short_description || "Subtítulo da página"}</p></div><div className="mt-5 space-y-2">{links.filter((link) => link.active && !socialTypes.includes(link.link_type)).map((link) => { const color = link.button_color || (link.highlighted ? draft.highlight_color : draft.button_color); const filled = link.highlighted || draft.button_variant === "filled"; return <div key={link.id} className={`flex min-h-11 items-center justify-center gap-2 px-4 text-center text-xs font-bold ${radius}`} style={filled ? { backgroundColor: color, color: "white" } : { borderColor: color, borderWidth: draft.button_border_width, color }}><IconFor type={link.link_type} />{link.title}</div>; })}</div><div className="mt-4 flex justify-center gap-3 text-slate-700">{links.filter((link) => link.active && socialTypes.includes(link.link_type)).map((link) => <IconFor key={link.id} type={link.link_type} className="size-5" />)}</div>{draft.capture_enabled && <div className="mt-5 rounded-2xl bg-white/85 p-4 shadow-sm backdrop-blur"><strong className="text-sm">{draft.capture_config.title || "Quero receber novidades"}</strong><div className="mt-3 space-y-2">{formFields.map((field) => <div key={field.key} className="h-9 rounded-lg border border-slate-200 bg-white px-3 py-2 text-[10px] text-slate-400">{field.label}{field.required ? " *" : ""}</div>)}</div><div className="mt-3 rounded-lg py-2.5 text-center text-[10px] font-bold text-white" style={{ backgroundColor: draft.form_button_color }}>{draft.capture_config.button_text || "Quero me cadastrar"}</div></div>}<p className="mt-5 text-center text-[9px] text-slate-500">{draft.footer_text || "Powered by OctaReview"}</p></div></div></aside>;
}
